...
export default class DraftManagerPlugin extends Plugin {
	...
	async onload() {
		...
		this.addCommand({id:"UpdatePage",name:"Update Page", editorCallback: async(editor:Editor,ctx:MarkdownFileInfo)=>{
			...
			const [continueForward, currentDraft]= extractCurrentDraft(editor.getValue(),draftNum)
			if(!continueForward){
				new Notice("Cannot find current drafts heading " + createDraftTitle(draftNum))
				return;
			}
			let newContent = "";
			// For the moment I am not allowing people to use different styles but that will be a future update.
			if(draftStyle =="Peterson"){
				// Create draft
				if(draftNum>1){
					newContent = createPetersonDraft(currentDraft,draftNum,this.settings.defaultFolderConditions)
				} else{
					newContent = createFromPetersonFirstDraft(currentDraft,this.settings.defaultFolderConditions)
				}
			}
			else if(draftStyle =="Blank") {
				const newLine = "\n";
				const newTitle = createDraftTitle(draftNum +1)
